...
usage() {
  cat <<'USAGE'
Usage: scripts/export-chatgpt-00-identity-zip.sh [options]
...
log_path="${log_dir}/chatgpt-00-identity-export-$(date -u +%Y%m%dT%H%M%SZ).log"
...
ensure_dir "${tmp_root}"
tmp_dir="$(mktemp -d "${tmp_root%/}/chatgpt-00-identity-export.XXXXXX")"
...
# export-chatgpt-00-identity-zip.sh EOF
